...
<template>
  ...
  >
    ...
    >
      ...
      <div style="width: 40em; margin: auto">
        ...
        <p>
          ...
          study, we will track the position of your mouse on screen.
          We will also track your responses to the questions.
          The data from this study may be presented at scientific conferences and
    ...
    <InstructionScreen :title="'Instructions'">
      ...
      <p>
        The sentences are taken from the transcript of a conversation taking place over text.
        <img src="../img/texting.jpeg" />
      </p>
      </div>

      <div v-if="condition==1">
      <p>
        The sentences are taken from the transcript of a spoken conversation transcribed by speech-to-text software.
        <img src="../img/verbal.jpeg" />
      </p>
      ...
      <Screen :title="'Instructions'">
        <p>
        What is the source of the sentences?
      </p>
      <MultipleChoiceInput
        :response.sync="$magpie.measurements.source"
        orientation="vertical"
        :options="['typed conversation', 'machine-generated transcript of conversation']"
      />
      <div v-if="$magpie.measurements.source">
      <button
        style="bottom: 30%; transform: translate(-50%, -50%)"
        @click="$magpie.addTrialData({sourceCondition: condition});
    ...
    <InstructionScreen :title="'Instructions'">
      <p>
        ...
        including from people with language impairments, so it may contain mistakes.
      </p>
      <div v-if="condition==0"> <!--TYPOS mentioned-->
      <p>
        After reading each sentence, you will then be prompted to select what is closest to
        what you think the intended sentence was. There may be errors. 
        There may be many ways to interpret a sentence, so just
        use your best judgment -- there's not one right answer.
        You will see some examples on the next page.
      </p>
    </div>
    <div v-if="condition==1"> <!--MISPRONUNCIATIONS mentioned-->
      <p>
        After reading each sentence, you will then be prompted to select what is closest to
        what you think the intended sentence was. Please correct any mistakes you found
        in the sentence. These could be wrong words. 
        There may be many ways to interpret a sentence, so just
        use your best judgment -- there's not one right answer.
        You will see some examples on the next page.
      </p>
    ...
    <InstructionScreen :title="'Instructions'">
      ...
      <table>
        ...
        <tbody>
          ...
          <tr>
            <td>my favorite desert is ice creme .</td>
            <td width="30" align="center">→</td>
            <td>my favorite dessert is ice cream .</td>
          </tr>
          <tr>
            <td>the boy's cat sat on the nat .</td>
            <td width="30" align="center">→</td>
      ...
      <br /><br />
      <ul>
        <li>If there's nothing wrong with the sentence, just select it.</li>
        <li>
          If there are mistakes, select what you think the person likely meant.
        </li>
      </ul>
    </InstructionScreen>
    ...
    <template v-for="(trial, i) of practiceTrials">
      ...
      >
        <Slide>
          ...
          >
            <template v-for="(word, index) of trial.error_sent.split(' ')">
          ...
          <div v-if="!showFirstDiv" class="userInput">
            <p>
              Please select what you think the intended sentence was, using your
            ...
            <MultipleChoiceInput
              ...
              orientation="vertical"
              :options="[ trial.error_sent, trial.typo_sent, trial.misspeak_sent ]"
            />
          </div>
    ...
    <InstructionScreen :title="'Practice Complete'">
      <p>
        ...
        experiment. Remember, there isn't one right answer, so just use your
        best judgment!
      </p>
    </InstructionScreen>
        ...
        <Slide>
          ...
          <div v-if="!showFirstDiv" class="userInput">
            ...
            <MultipleChoiceInput
              :response.sync="$magpie.measurements.response"
              orientation="vertical"
              :options="shuffleTrialArguments"
            />
            -->
          </div>

          <button
            v-if="$magpie.measurements.response == trial.error_sent"
            @click="
              toggleDivs();
              $magpie.addTrialData({
                TrialId: i,
          ...
          <button
            v-if="$magpie.measurements.response == trial.typo_sent"
            @click="
              toggleDivs();
              $magpie.addTrialData({
                TrialId: i,
          ...
          <button
            v-if="$magpie.measurements.response == trial.misspeak_sent"
            @click="
              toggleDivs();
              $magpie.addTrialData({
                TrialId: i,
    ...
    <Screen>
      ...
      <div
        v-if="
          $magpie.measurements.device &&
          $magpie.measurements.hand
        "
      >
      <button
        style="bottom: 30%; transform: translate(-50%, -50%)"
...
export default {
  ...
  data() {
    ...
    const updatedShuffledItems = shuffledItems.map( trial => {
      ...
      return {
        ...
        condition_id: col,
        experiment_id: "pilot",
        ordering: _.shuffle([trial.error_sent, trial.typo_sent, trial.misspeak_sent])
      };
    });
  ...
  methods: {
    /*
    shuffleTrialArguments() {
      return _.shuffle(['Trial Arg 1', 'Trial Arg 2', 'Trial Arg 3']);
    },
    shuffleArguments(arguments) {
      return _.shuffle(arguments);
    },
    */
    changeBack() {
